...
const ScrollChatBox = ({ messages, activechat, typing }) => {
    ...
    return (
        <>
            ...
            {/* for mobile screen  */}
            <ScrollableFeed className='md:!hidden  !h-full !w-full !scrollbar-none !transition-all !mt-3' ref={scrollRef2} onScroll={(isAtBottom2) => setIsAtBottom2(isAtBottom2)} >

                {/* // scroll to bottom button  */}
                {!isAtBottom2 && <div className='absolute bottom-5 right-10 z-[100] bg-gray-100 shadow-md transition-all rounded-full'>
                    <IconButton color='secondary' size="small" onClick={scrollToBottom2}><KeyboardDoubleArrowDownIcon fontSize='1.2rem' /></IconButton>
                </div>
                }
                ...
                {(typing && !activechat?.isGroupChat) && <div className='w-14 h-14 md:!w-20 md:!h-28'>
                    <Lottie style={{ marginBottom: 5, height: "100%", width: "100%" }} loop={true} autoPlay={true} animationData={animationData} rendererSettings={{ preserveAspectRatio: "xMidYMid slice" }} />
                </div>
                }
