...
const ChatBox = ({ token, activechat, fetchMessages, allMessages, loading, errorMessage, setError, setMessages, socketConnected, setFetchAgain, fetchAgain }) => {
    ...
    return (
        ...
        <>
            ...
            {/* for mobile screen  */}
            <>
                <div className='md:!hidden flex flex-col h-[80vh]  w-full mx-auto overflow-y-auto absolute top-[12vh] z-30 px-[13px] transition-all  scrollbar-none '>
                    {loading ?
                        <div className='w-full h-full flex justify-center items-center transition-all '>
                    ...
                    {showEmojiPicker && <div className='!shadow-lg'> <Picker disableSearchBar pickerStyle={{ position: "absolute", bottom: 2, height: "10rem", border: "1.5px solid #7E22CE" }} disableAutoFocus onEmojiClick={addEmojiToText} /></div>}
                </div>
                {/* message sending textfield  */}
                {activechat?.users?.find((auser) => auser._id === user?._id) ? <div className='md:hidden w-full h-[8vh] mx-auto  bg-white p-2 drop-shadow-md border-t-[1.5px]  border-purple-700  absolute bottom-0 z-30 flex items-center'>
                    <IconButton sx={{ marginRight: "12px", marginLeft:"5px" }} size="small" onClick={() => setShowEmojiPicker(!showEmojiPicker)} ><EmojiEmotionsIcon color='success' fontSize='1.1rem' /></IconButton>
                    <input id='message' autoComplete='off' name='message' value={message} placeholder="Message.." onChange={typingHandler} className="py-2 px-3 w-[80%] text-sm font-nunito  outline-none border border-slate-300 flex-1 " onKeyDown={sendMessage} />
